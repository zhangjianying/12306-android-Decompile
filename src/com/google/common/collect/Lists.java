...
public final class Lists
{
  ...
  private static class Partition<T> extends AbstractList<List<T>>
  {
    ...
    public int size()
    {
      return (-1 + (this.list.size() + this.size)) / this.size;
    }
  }
  ...
  private static class ReverseList<T> extends AbstractList<T>
  {
    ...
    public ListIterator<T> listIterator(int paramInt)
    {
      ...
      return new ListIterator(this.forwardList.listIterator(i))
      {
        boolean canRemove;
        boolean canSet;
  ...
  private static final class StringAsImmutableList extends ImmutableList<Character>
  {
    ...
    public UnmodifiableListIterator<Character> listIterator(int paramInt)
    {
      return new AbstractIndexedListIterator(size(), paramInt)
      {
        protected Character get(int paramInt)
        {
          return Character.valueOf(Lists.StringAsImmutableList.this.string.charAt(paramInt));
        }
      };
    }
    ...
    implements Serializable
  {
    ...
    public ListIterator<T> listIterator(int paramInt)
    {
      return new ListIterator(this.fromList.listIterator(paramInt))
      {
        public void add(T paramT)
        {
